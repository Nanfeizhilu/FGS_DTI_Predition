...
def built_multiple_similarity_matrix2(simfile_name, length):
    ns = 1
    sim_files = [simfile_name]
    sims = np.zeros(shape=(ns, length, length)) # sims[i] is the i-th similarity matrix
    for i in range(ns):
        sims[i] = np.loadtxt(simfile_name, delimiter='\t',dtype=np.float64,skiprows=1, usecols=range(1,length+1))
    return sim_files, sims
#----------------------------------------------------------------------------------------    
...
def load_datasets_single_multi_sims(dataset, file_path, method_d='', method_t=''):
    """ multiple similarities data 
        method_d and method_t is the name(type) of similarity used 
        if method_d(t) is a file name with suffix '.txt, it denotes single similairty
        else it means a type of similarities, e.g. all, lowLimb4, Original
    """
    if dataset[-1] == '1' or dataset[-1]=='4': # the updated interaction matrix
        sim_data_name = dataset[:-1]
    else:
    ...
    Dsimfile_path = os.path.join(data_path, 'Dsim')
    if method_d[-4:] == '.txt':
        simfile_name = os.path.join(Dsimfile_path, method_d)
        Dsim_names, Dsims = built_multiple_similarity_matrix2(simfile_name, len(drugs))
    else:
        if method_d == 'all':
            Dsim_name_file = os.path.join(Dsimfile_path,'allDsim_files.txt')  # 'allDsim_files.txt'
        elif method_d == '' or method_d == 'FSS':
            Dsim_name_file = os.path.join(Dsimfile_path,'selected_Dsim_files.txt') # forward feature selection (DTiGEMS+ paper) 
        else:
            Dsim_name_file = os.path.join(Dsimfile_path,'selected_Dsim_files_'+method_d+'.txt')
        Dsim_names, Dsims = built_multiple_similarity_matrix(Dsimfile_path, Dsim_name_file, len(drugs))
    
    Tsimfile_path = os.path.join(data_path, 'Tsim')
    if method_t[-4:] == '.txt':
        simfile_name = os.path.join(Tsimfile_path, method_t)
        Tsim_names, Tsims = built_multiple_similarity_matrix2(simfile_name, len(targets))
    else:
        if method_t == 'all':
            Tsim_name_file = os.path.join(Tsimfile_path,'allTsim_files.txt')  # 'allTsim_files.txt'
        elif method_t == '' or method_t == 'FSS':
            Tsim_name_file = os.path.join(Tsimfile_path,'selected_Tsim_files.txt') # forward feature selection (DTiGEMS+ paper)
        else:
            Tsim_name_file = os.path.join(Tsimfile_path,'selected_Tsim_files_'+method_t+'.txt')
        Tsim_names, Tsims = built_multiple_similarity_matrix(Tsimfile_path, Tsim_name_file, len(targets))
